//This class responsible about the users requests 
...
router.get('/verify/getFavorite',function(req,res) {
    ...
    DButilsAzure.execQuery("select pointID, priority from UserFavorite WHERE userName ='"+ userName+"'  ORDER BY [datePost] DESC;")
            .then((response) => {
                res.send(response);
            })
           .catch((err)=>{
...
if (result.length<1)
     {
        DButilsAzure.execQuery("INSERT INTO UserFavorite ([userName] ,[pointID], [datePost], [priority]) VALUES ('"+userName+"','"+pointID+"','"+date+"','"+priority+"')") 
        .then(function (result) {
            res.send(result);
        })
        .catch((err)=>{
...
//update fav
router.put('/verify/updateFavorite',function(req,res) {
    var token = req.decoded;
    var userName = token.payload.userName;
    var pointID = req.body.pointID;
    var priority = req.body.priority;

    DButilsAzure.execQuery("update UserFavorite set priority='"+priority+"' where userName ='"+userName+"' AND pointID='"+pointID+"';")
        .then(function (result) {
            res.send(result);
        })
        .catch((err)=>{
